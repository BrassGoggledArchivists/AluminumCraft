...
public class ALItemBucketAluminium extends Item implements ITextureProvider
{
    ...
    public ALItemBucketAluminium(int i, int j)
    {
        super(i);
        maxStackSize = 1;
        isFull = j;
        ...
        if(movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
        {
            ...
            if(isFull == 0)
            {
                ...
                if(world.getBlockMaterial(i, j, k) == Material.water && world.getBlockMetadata(i, j, k) == 0)
                {
                    ...
                    return new ItemStack(mod_Aluminium.aluminiumBucketWater);
                }
                if(world.getBlockMaterial(i, j, k) == Material.lava && world.getBlockMetadata(i, j, k) == 0)
                {
                    world.setBlockWithNotify(i, j, k, 0);
                    return new ItemStack(mod_Aluminium.aluminiumBucketLava);
                }
            } else
            {
                if(isFull < 0)
                {
                    return new ItemStack(mod_Aluminium.aluminiumBucket);
                }
